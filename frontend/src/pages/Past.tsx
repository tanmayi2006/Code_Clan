...
function Past() {
  const [currentSlide, setCurrentSlide] = useState(0);
  const [selectedCategory, setSelectedCategory] = useState<typeof categories[number]>('all');
  const [activeSong, setActiveSong] = useState<Show | null>(null);
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(to bottom right, #0A0A1A, #0D152C)' }}>
      ...
      {/* Render the Seekbar when an active song is selected */}
      {activeSong && <Seekbar song={{ 
          title: activeSong.title, 
          image: activeSong.image, 
          links: { audio: activeSong.links.spotify || activeSong.links.youtube || '' } 
